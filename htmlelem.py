...
#	from htmlelem import get_element
#	data = get_element(httpdata)
#	
#	stack = [data[0]]
#	while len(stack) > 0:
#		top = stack.pop()
...
class HTMLElement:
	def __init__(self, elem = None):
		...
		remove_char = lambda x: x.strip("\ \n\r\t\"=")

		self.name = remove_char(elem[:endname])
...
def find_next_element(html, begin):
...
def get_element(html):
	...
	pos = find_next_element(html, 0)
	while is_elem_valid(pos[2]) == False and pos[2] != None:
		pos = find_next_element(html, pos[1])
	...
	while len(elemstack) > 0:
		...
		nextPos = find_next_element(html, pos[1])
