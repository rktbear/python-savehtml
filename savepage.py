...
def rel_to_abs_path(relurl, absurl):
	final = relurl
...
def handle_html(data, dataurl):
	...
	def add_html_link(absurl, relurl, localpath, process):
	...
	if "img" in allelements:
		for e in allelements["img"]:
			...
			relurl = e.attr["src"]
			absurl = rel_to_abs_path(relurl, dataurl)

			localpath = "imgh"+str(filecount)+get_extension(absurl)
			filecount = filecount + 1

			add_html_link(absurl, relurl, localpath, handle_raw)
	...
	if "link" in allelements:
		for e in allelements["link"]:
			...
			add_html_link(absurl, relurl, localpath, handle_css)
	...
	if "style" in allelements:
		for e in allelements["style"]:
			...
			resultHandle = handle_css(stylevalue, dataurl)
...
def handle_raw(data, dataurl):
	return (data, [])

def handle_css(data, dataurl):
	global filecount
	global allLinks
	...
	while start != 0:
		...
		localpath = "imgcss"+str(filecount)+get_extension(relurl)
		filecount = filecount + 1

		if (absurl in allLinks) == False:
			allLinks[absurl] = localpath
			links.append((absurl, localpath, handle_raw))
...
queue.append((URL_HTML, "main.html", handle_html))
